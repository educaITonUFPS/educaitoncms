...
<!--
      <div style="text-align:center;">
-->
        <h2 id="tituloaddbox" style="color:#fff;"><?php if ($_idedicion) { echo "Editar enlace Id. ".$_idedicion; } else { echo "Agregar un nuevo enlace"; } ?></h2>
<!--
      </div>
-->
    </div>
...
-->
        <form name="annadir" action="swapis.php?<?php echo $_SERVER['QUERY_STRING']; ?>" method="post"<?php if ($_tipoenlace > 1) { echo " enctype=\"multipart/form-data\""; } ?>>
          <input type="hidden" name="accion" value="nenlace">
          <input type="hidden" name="estado" value="">
          <input type="hidden" name="conimagen" value="<?php if ($_tipoenlace == 1) { echo "0"; } else { echo "1"; } ?>">
...
<?php
          ...
          if ($_tipoenlace == 1) {
?>
          <p class="pnormal">Descripci&oacute;n:<br><textarea class="ufps-textarea" name="etiqueta"><?php if ($_idedicion) { echo utf8_encode($_campos[4]); } ?></textarea></p>
<?php
          } else {
?>
          <p class="pnormal"><?php if ($_idedicion) { echo "<span id=\"ImagenActual\">Imagen actual:<br><img src=\"".$_campos[4]."\"><br><br>Nueva i</span>"; } else { echo "I"; } ?>magen<?php if ($_imgheight) { ?><span style="font-style:italic;"> (alto m&iacute;nimo permitido: <?php echo $_imgheight; ?>px)</span><?php } ?>:<br><input type="file" name="etiqueta" id="uploadfile"></p>
<?php
          }
?>
          <p class="pnormal">URL: <input type="text" class="ufps-input-short ufps-input" name="url" style="width:40%;" onblur="checkurl(document.forms['annadir'].url.value);"<?php if ($_idedicion) { if ($_campos[5] != "javascript:;") { if (!is_numeric($_campos[5])) { echo " value=\"".$_campos[5]."\""; } } } ?>> <span id="urlresult" style="margin-right:10px;"><img id="urlstatus" src="<?php if ($_idedicion) { echo urlImage($_campos[5], $_idedicion); } else { echo "rsc/img/gray.png"; } ?>"></span> <input type="checkbox" name="afuera"<?php if ($_idedicion) { if ($_campos[6] == 1) { echo " checked"; } } ?>> URL abre en nueva pesta&ntilde;a</p>
<?php
