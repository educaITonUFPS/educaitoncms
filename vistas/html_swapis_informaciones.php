...
<!--
      <div style="text-align:center;">
-->
        <h2 id="tituloaddbox" style="color:#fff;"><?php if ($_idedicion) { echo "Editar informaci&oacute;n Id. ".$_idedicion; } else { echo "Agregar una nueva informaci&oacute;n/m&oacute;dulo"; } ?></h2>
<!--
      </div>
-->
    </div>
...
-->
        <form name="annadir" action="swapis.php?<?php echo $_SERVER['QUERY_STRING']; ?>" method="post"<?php if ($_esnoticia) { echo " enctype=\"multipart/form-data\""; } ?>>
          <input type="hidden" name="accion" value="ninformaciones">
          <input type="hidden" name="estado" value="">
          <input type="hidden" name="conimagen" value="<?php if ($_esnoticia == 1) { echo "1"; } else { echo "0"; } ?>">
...
?>
          ...
          <input type="hidden" name="activo" value="<?php if ($_campos[4] == 1) { echo "1"; } else { echo "0"; } ?>">
<?php
          }
          if ($_esnoticia == 1) {
?>
          <p class="pnormal">Fecha de la informaci&oacute;n: <input type="text" class="ufps-input-short ufps-input" name="fecha" id="fecha"<?php if ($_idedicion) { echo " value=\"".$_campos[5]."\""; } ?>></p>
...
?>
          <p class="pnormal">T&iacute;tulo: <input type="text" class="ufps-input-medium ufps-input" name="titulo"<?php if ($_idedicion) { echo " value=\"".utf8_encode($_campos[6])."\""; } ?>></p>
<?php
          if ($_esnoticia == 1) {
?>
          <p class="pnormal"><?php if ($_idedicion) { echo "<span id=\"ImagenActual\">Imagen actual:<br><img src=\"".$_campos[7]."\" style=\"max-height:80px;\"><br><br>Nueva i</span>"; } else { echo "I"; } ?>magen<?php if ($_minwidth) { ?><span style="font-style:italic;"> (ancho m&iacute;nimo permitido: <?php echo $_minwidth; ?>px; dimensiones recomendadas: <?php echo $_imagewidth; ?>px de ancho o <?php echo $_imageheight; ?>px de alto)</span><?php } ?>:<br><input type="file" name="etiqueta" id="uploadfile"><br>Si es necesario para visualizar en la p&aacute;gina principal, cortar imagen en: <input type="radio" name="crop" value="1" checked> Borde superior <input type="radio" name="crop" value="2"> Centro <input type="radio" name="crop" value="3"> Borde inferior</p>
          <p class="pnormal" style="margin-top:25px;">Descripci&oacute;n inicial:<br><textarea class="ufps-textarea" id="texto" name="texto"><?php if ($_idedicion) { echo utf8_encode($_campos[9]); } ?></textarea></p>
<?php
          } else {
?>
          <p class="pnormal">Ancho en columnas: <input type="text" class="ufps-input-number ufps-input-short ufps-input" name="wide"<?php if ($_idedicion) { echo " value=\"".$_campos[2]."\""; } ?>></p>
...
?>
          <p class="pnormal" style="margin-top:25px;"><?php if ($_esnoticia == 1) { echo "Ampliaci&oacute;n"; } else { echo "Descripci&oacute;n"; } ?>:<br><textarea class="ufps-textarea" id="textomas" name="textomas"><?php if ($_idedicion) { echo utf8_encode($_campos[10]); } ?></textarea></p>
<?php
