...
<?php
    ...
    if (!$_solocontenido) {
?>
            <div id="proximasContent" class="gdl-custom-sidebar gdl-custom-sidebar<?php if ($_nofirst == 1) { echo "-nofirst"; } ?>">
<?php
    }
  }
?>
              <h3 class="gdl-custom-sidebar-title-m"><?php if (isset($_myactividades->etiqueta)) { echo $_myactividades->etiqueta; } ?><div class="ufps-tooltip" style="float:right;"><span style="float:right;"><a href="./index.php?modulo=calendarios"><img src="rsc/img/calendar.png"></a></span><span class="ufps-tooltip-content-left" style="font-size:14px; font-weight:normal; margin-right:5px;">Ir al calendario</span></div></h3>
              <div id="browseweek" class="browseweek" style="background:#424242; min-height:25px; width:<?php if ($_parametroP == 'top') {  echo "95"; } elseif ($_parametroP == 'right') { echo "100"; } ?>%; margin:auto; margin-bottom:0px;"><div class="ufps-tooltip"><div style="margin-left:2px; margin-right:2px; float:left; top:0px; left:0px; width:25px;"><img src="rsc/img/rewind.png" style="cursor:pointer;" onclick="WeekDirection(<?php if (isset($_GET['semana'])) { if ($_GET['semana']) { echo $_GET['semana']; } else { echo "0"; } } else { echo "0"; } ?>,-1,'<?php echo $_parametroP; ?>');"></div><span class="ufps-tooltip-content-right">Semana anterior</span></div><div class="ufps-tooltip" style="float:right;"><div style="margin-left:2px; margin-right:2px; float:right; top:0px; right:0px; width:25px;"><img src="rsc/img/forward.png" style="cursor:pointer;" onclick="WeekDirection(<?php if (isset($_GET['semana'])) { if ($_GET['semana']) { echo $_GET['semana']; } else { echo "0"; } } else { echo "0"; } ?>,+1,'<?php echo $_parametroP; ?>');"></div><span class="ufps-tooltip-content-left">Semana siguiente</span></div></div>
<?php
  if ($_enlaces[0] != 'error') {
    if ($_parametroP == 'top') {
?>
