...
<!--
      <div style="text-align:center;">
-->
        <h2 id="tituloaddbox" style="color:#fff;"><?php if ($_idedicion) { echo "Editar galer&iacute;a Id. ".$_idedicion; } else { echo "Agregar una nueva galer&iacute;a"; } ?></h2>
<!--
      </div>
-->
    </div>
...
?>
          ...
          <input type="hidden" name="activo" value="<?php if ($_campos[2] == 1) { echo "1"; } else { echo "0"; } ?>">
<?php
          }
?>
          <p class="pnormal">Fecha: <input type="text" class="ufps-input-short ufps-input" name="fecha" id="fecha"<?php if ($_idedicion) { echo " value=\"".$_campos[3]."\""; } ?>></p>
          <p class="pnormal">T&iacute;tulo: <input type="text" class="ufps-input-medium ufps-input" name="titulo"<?php if ($_idedicion) { echo " value=\"".utf8_encode($_campos[5])."\""; } ?>></p>
          <p class="pnormal"><?php if ($_idedicion) { echo "<span id=\"ImagenActual\">Imagen actual:<br><img src=\"".$_campos[4]."\"><br><br>Nueva i</span>"; } else { echo "I"; } ?>magen<?php if ($_ThumbWidth) { ?><span style="font-style:italic;"> (ancho m&iacute;nimo permitido: <?php echo $_ThumbWidth; ?>px)</span><?php } ?>:<br><input type="file" name="etiqueta" id="uploadfile"><br>Si es necesario cortar la imagen, hacerlo en: <input type="radio" name="crop" value="1" checked> Borde superior/izquierdo <input type="radio" name="crop" value="2"> Centro <input type="radio" name="crop" value="3"> Borde inferior/derecho</p>
          <p class="pnormal">Descripci&oacute;n:<br><textarea class="ufps-textarea" name="descripto"><?php if ($_idedicion) { echo utf8_encode($_campos[6]); } ?></textarea></p>
...
?>
    ...
    <div class="ufps-modal-header ufps-modal-header-red">
    <span class="ufps-modal-close">&times;</span>
        <h2 id="tituloaddbox" style="color:#fff;"><?php if ($_idedicion) { echo "Editar imagen Id. ".$_idedicion; } else { echo "Agregar una nueva imagen"; } ?></h2>
    </div>
    <div class="ufps-modal-body" style="background-color: #cecece;">
        <form name="annadir" action="swapis.php?<?php echo $_SERVER['QUERY_STRING']; ?>" method="post" enctype="multipart/form-data">
          <input type="hidden" name="accion" value="gagrid">
...
?>
          <p class="pnormal"><?php if ($_idedicion) { echo "<span id=\"ImagenActual\">Imagen actual:<br><img src=\"".$_campos[1]."\" style=\"max-height:80px;\"><br><br>Nueva i</span>"; } else { echo "I"; } ?>magen<?php if ($_imagewidth) { ?> <span style="font-style:italic;">(dimensiones recomendadas: <?php echo $_imagewidth; ?>px de ancho o <?php echo $_imageheight; ?>px de alto)</span><?php } ?>:<br><input type="file" name="etiqueta" id="uploadfile"></p>
          <p class="pnormal" style="margin-top:25px;">Descripci&oacute;n de la imagen:<br><textarea class="ufps-textarea" name="descripto"><?php if ($_idedicion) { echo utf8_encode($_campos[2]); } ?></textarea></p>
