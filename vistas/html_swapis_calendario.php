          ...
          <h2>Administraci&oacute;n de actividades para calendarios</h2>
          </div>
<?php
  $_mallainicial = 0 ;
  $_tipocalendario = 0;
  ...
  if (isset($_GET['tipocalendario'])) {
    if ($_GET['tipocalendario']) {
      $_tipocalendario = $_GET['tipocalendario'];
    }
  }
  if (isset($_GET['ixp'])) {
    if ($_GET['ixp']) {
...
?>
          ...
          <div style="float:right; margin-right:30px; margin-bottom:30px;"><button class="ufps-btn" onclick="prepararaddbox()">Crear nueva Actividad</button></div>
          ...
          <script>
            function estado(_id, _estado, _mallainicial) {
              ...
              if (confirm("Va a proceder a "+texto+" un registro de actividad") == true) {
                document.forms['acciones'].accion.value = 'acestado';
            ...
            function editar(_id, _mallainicial) {
              document.forms['acciones'].accion.value = 'aceditar';
              document.forms['acciones'].id.value = _id;
              document.forms['acciones'].mallainicial.value = _mallainicial;
              document.forms['acciones'].submit();
            }
            function prepararaddbox() {
              ...
              document.forms['annadir'].accion.value = 'nactividad';
              ...
              for (var i = 0; i < document.forms['annadir'].elements.length; i++) {
                if (document.forms['annadir'].elements[i].type == 'checkbox') {
                  if (document.forms['annadir'].elements[i].name.substring(0,3) == 'cal') {
                    document.forms['annadir'].elements[i].checked = false;
                  }
                }
              }
              document.getElementById("tituloaddbox").innerHTML = "Agregar una nueva actividad";
...
<?php
  $_myActividades = new Actividades($_tipocalendario);
  $_totalregistros = $_myActividades->totalActividades();
  $_enlaces = $_myActividades->obtenerPagina($_mallainicial, $_actxpag);
  if ($_enlaces != 'error') {
    paginador("calendario",$_mallainicial, $_totalregistros, $_actxpag);
  }
  filasporpagina($_actxpag);
  mallacalendario($_mallainicial, $_enlaces);
  filasporpagina($_actxpag);
  if ($_enlaces != 'error') {
    paginador("calendario",$_mallainicial, $_totalregistros, $_actxpag);
  }
  $_enlaces = $_myActividades->obtenerTipoCalendario(0);
  $_naturaleza = 0;
  $_campos = array();
  if (isset($_POST['accion'])) {
    if ($_POST['accion'] == 'aceditar') {
      $_naturaleza = $_POST['id'];
      $_campos = $_myActividades->obtenerCamposXaEdicion($_naturaleza);
    }
  }
  cuadromascalendario($_enlaces, $_naturaleza, $_campos);

  function mallacalendario($limite_inicial, $_enlaces) {
    if ($_enlaces[0] != "error") {
?>
...
<?php
      ...
      echo "<div style=\"clear:both; text-align:center;\"><h2 style=\"color:red;\">No hay registros en la tabla de actividades</h2></div>";
    }
  }
  function cuadromascalendario($_enlaces, $_idedicion, $_campos) {
?>
    <script>
          ...
          if (document.forms['annadir'].fechini.value <= document.forms['annadir'].fechfin.value) {
            if (document.forms['annadir'].descripto.value.length > 0) {
              ...
              if (_calendariomarcado == 1) {
                ...
                alert("Debe asociarse al menos un calendario a la actividad");
              }
            } else {
...
<!--
      <div style="text-align:center;">
-->
        <h2 id="tituloaddbox" style="color:#fff;"><?php if ($_idedicion) { echo "Editar actividad Id. ".$_idedicion; } else { echo "Agregar una nueva actividad"; } ?></h2>
<!--
      </div>
-->
    </div>
...
-->
        <form name="annadir" action="swapis.php?<?php echo $_SERVER['QUERY_STRING']; ?>" method="post">
          <input type="hidden" name="accion" value="nactividad">
          <input type="hidden" name="estado" value="">
<?php
          if ($_idedicion) {
?>
          <input type="hidden" name="id" value="<?php echo $_campos[0]; ?>">
          <input type="hidden" name="activo" value="<?php if ($_campos[8] == 1) { echo "1"; } else { echo "0"; } ?>">
<?php
          }
?>
          <p class="pnormal">Fecha inicial: <input type="text" class="ufps-input-short ufps-input" name="fechini" id="fechini"<?php if ($_idedicion) { echo " value=\"".$_campos[1]."\""; } ?>></p>
          <p class="pnormal">Fecha final: <input type="text" class="ufps-input-short ufps-input" name="fechfin" id="fechfin"<?php if ($_idedicion) { echo " value=\"".$_campos[2]."\""; } ?>></p>
          <p class="pnormal">Descripci&oacute;n:<br><textarea class="ufps-textarea" name="descripto"><?php if ($_idedicion) { echo utf8_encode($_campos[5]); } ?></textarea></p>
          <p class="pnormal">URL: <input type="text" class="ufps-input-short ufps-input" name="url" style="width:40%;" onblur="checkurl(document.forms['annadir'].url.value);"<?php if ($_idedicion) { echo " value=\"".$_campos[6]."\""; } ?>> <span id="urlresult" style="margin-right:10px;"><img id="urlstatus" src="<?php if ($_idedicion) { if ($_campos[6]) { echo "rsc/img/urlok.png"; } else { echo "rsc/img/gray.png"; } } else { echo "rsc/img/gray.png"; } ?>"></span> <input type="checkbox" name="afuera"<?php if ($_idedicion) { if ($_campos[7] == 1) { echo " checked"; } } ?>> URL abre en nueva pesta&ntilde;a</p>
          <p class="pnormal">Calendarios relacionados:<br>
<?php
    if ($_enlaces[0] != 'error') {
      for ($_a = 0; $_a < count($_enlaces); $_a++) {
        echo "<input type=\"checkbox\" name=\"cal".$_a."\" value=\"".$_enlaces[$_a][0]."\" style=\"margin-left:15px;\"";
        if ($_idedicion) {
      ...
      echo "<p><i>No hay tipos de calendarios definidos para asociar la actividad</i></p>";
    }
?>
          </p>
